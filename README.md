...
3. Crear una función llamada `filtro` que reciba un arreglo con números, dos valores de límite inferior y superior y una variable booleana y devuelva un arreglo con los números entre el intervalo incluyendo los valores de los límites si la variable booleana es verdadera o devuelva un arreglo con los números fuera del intervalo si la variable booleana es falsa, ejemplo `filtro([1, 2, 3, 5], 2, 3, True)` devuelve `[2, 3]`, `filtro([1, 2, 3, 5], 2, 3, False)` devuelve `[1, 5]`.

4. Crear una función llamada `conjunto` que tome un arreglo y devuelva un arreglo sin elementos repetidos, ejemplo `conjunto([1, 2, 1, 2, 3, 1, 2, 1, 3, 3, 2])` devuelve `[1, 2, 3]`. _Nota: El orden del conjunto no importa_.

5. Crear una función llamada `union` que tome dos arreglos y devuelva un conjunto equivalente a la unión de ellos, ejemplo `union([1, 2, 3], [1, 3, 5])` devuelve `[1, 2, 3, 5]`, `union([1, 2], [])` devuelve `[1, 2]` y `union([1, 1, 1, 1], [1, 1, 1])` devuelve `[1]`.

6. Crear una función llamada `interseccion` que tome dos arreglos y devuelva un conjunto equivalente a la intersección de ellos, ejemplo `interseccion([1, 2, 3, 4, 5], [1, 3, 5, 7])` devuelve `[1, 3, 5]`, `interseccion([1, 2], [3, 4])` devuelve `[]` e `interseccion([1, 1, 1], [1, 1, 2, 2])` devuelve `[1]`.

7. Crear una clase llamada `Linea` la cual se construya con los valores `x0, y0, x1, y1` correspondientes a dos puntos diferentes en el plano. Colocar un método que se llame `angulo()` que devuelva el ángulo de la recta del segundo punto respecto al primero `a = atan2(y1-y0, x1-x0)`. Colocar un método llamado `pendiente()` que devuelva la pendiente de la recta. Colocar un método que se llame `paralela_con(linea)` que reciba otra `linea` y devuelva verdadero o falso si son paralelas (use las pendientes para saber si son paralelas). Colocar un método llamado `perpendicular_con(linea)` que devuelva si es perpendicular con la otra recta (también se sabe según sus pendientes). __Bonus:__ Dibuje las recta con `Turtle` asignando el métodos `dibujar()` a la clase (El bonus vale por unas papas o refresco XD).
